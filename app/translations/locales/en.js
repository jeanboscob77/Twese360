const en = {
  // Navbar translations which includes nested submenu items
  home: "Home",
  whoWeAre: "Who We Are",
  contact: "Contact Us",
  services: {
    Food: {
      label: "Food",
      submenu: {
        "Ordering & Delivery": "Ordering & Delivery",
        "Local Restaurants": "Local Restaurants",
        "Delivery Options": "Delivery Options",
        "Events Catering": "Events Catering",
        "Decorators & Servers": "Decorators & Servers",
        "Personal Chefs": "Personal Chefs",
        "Busy Professionals": "Busy Professionals",
      },
    },
    Transport: {
      label: "Transport",
      submenu: {
        "Car Renting": "Car Renting",
        "With/Without Driver": "With/Without Driver",
        "Purchase Car": "Purchase Car",
      },
    },
    Weddings: {
      label: "Weddings",
      submenu: {
        "Event Management": "Event Management",
        "Music & Bands": "Music & Bands",
        Protocol: "Protocol",
        Transport: "Transport",
        "Food and drinks": "Food and Drinks",
        Clothes: "Clothes",
        "Save the Date": "Save the Date",
        "Wedding Invitation": "Wedding Invitation",
        "Gifts cover & Badges": "Gifts Cover & Badges",
      },
    },
    Rentals: {
      label: "Rentals",
      submenu: {
        "Apartments & Houses": "Apartments & Houses",
        House: "House",
        "Office & Space": "Office & Space",
      },
    },
    Tourism: {
      label: "Tourism",
      submenu: {
        "Hotel Booking": "Hotel Booking",
        Guidance: "Guidance",
        "Gorilla Trekking": "Gorilla Trekking",
        "City Tours": "City Tours",
        "Nature Walks": "Nature Walks",
      },
    },
    "Research Services": {
      label: "Research Services",
      submenu: {
        "Field Enumerators": "Field Enumerators",
        "Data Analysis": "Data Analysis",
        "Survey Design & Reports": "Survey Design & Reports",
        "Software Traing": "Software Training",
        "Data Entry": "Data Entry",
        "Dashboard Design": "Dashboard Design",
      },
    },
  },

  // Home page translations
  homepage: {
    heroTitle: "From Daily needs to big moments, we’ve got you covered",
    heroSubtitle:
      "We provide a wide range of services to make your life easier.",
    servicesTitle: "Our Services",
    aboutTitle: "About Us",
    aboutText:
      "Our company is dedicated to providing top-notch services across various industries. We focus on quality, reliability, and customer satisfaction.",
    ctaTitle: "Get in Touch Today",
    ctaSubtitle: "Reach out to us and let’s discuss how we can help you.",
  },

  //contact page translation
  contactPage: {
    title: "Contact Us",
    successMessage: "Thank you! We will contact you soon.",
    form: {
      name: "Name",
      email: "Email",
      number: "Phone Number (Optional)",
      message: "Message",
      submit: "Send Message",
    },
    address: {
      email: "info@example.com",
      phones: ["+250 788 123 456", "+250 789 654 321"],
      location: "Kigali, Rwanda",
      website: "www.twese360.rw",
    },
    labels: {
      email: "Email",
      phone: "Phone",
      location: "Location",
      website: "Website",
    },
  },
};

export default en;
